...
joint.data <- data.frame(joint.data, sub.y)
...
# get mean value 
table.split.by.y.subject <- t(sapply(l, function(x) sapply(x, mean)))

# drop useless column
drops <- c("y", "subject", "sub.y")
table.split.by.y.subject <- table.split.by.y.subject[,!(colnames(table.split.by.y.subject) %in% drops)]
